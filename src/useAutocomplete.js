...
export default function useAutocomplete(props) {
  ...
  const { current: isInputValueControlled } = React.useRef(
    inputValueProp != null
  );
  ...
  const selectNewValue = (event, newValue) => {
    if (multiple) {
      ...
      // console.log("itemIndex", itemIndex, value, item);
      const itemIndex = value.findIndex((v) => v === item);
  ...
  const handleOptionClick = (event) => {
    const index = Number(event.currentTarget.getAttribute("data-option-index"));
    console.log("handleOptionClick", index, filteredOptions[index]);
